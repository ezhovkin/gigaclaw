---
name: setup
description: Run initial GigaClaw setup. Use when user wants to install dependencies, configure Telegram bot, register their main channel, or start the background services. Triggers on "setup", "install", "configure gigaclaw", or first-time setup requests.
---
...
> GigaClaw needs a container runtime for isolated agent execution. You have two options:
>
...
**Note:** GigaClaw automatically starts the Apple Container system when it launches, so you don't need to start it manually after reboots.
...
Build the GigaClaw agent container:

```bash
./container/build.sh
```

This creates the `gigaclaw-agent:latest` image with Node.js, Chromium, Claude Code CLI, and agent-browser.
...
if which docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
  echo '{}' | docker run -i --entrypoint /bin/echo gigaclaw-agent:latest "Container OK" || echo "Container build failed"
else
  echo '{}' | container run -i --entrypoint /bin/echo gigaclaw-agent:latest "Container OK" || echo "Container build failed"
fi
```
...
> You need a Telegram Bot Token. To create one:
>
> 1. Open Telegram and find @BotFather
> 2. Send `/newbot` and follow the prompts
> 3. Copy the bot token
> 4. Add it to `.env` as `TELEGRAM_BOT_TOKEN=your-token-here`
> 5. Also add your Telegram user ID: `TELEGRAM_ALLOWED_USER_IDS=your-user-id`
...
> What trigger word do you want to use? (default: `Neo`)
>
> Messages starting with `@TriggerWord` will be sent to Claude.

If they choose something other than `Neo`, update it in these places:

1. `groups/CLAUDE.md` - Change "# Neo" and "You are Neo" to the new name
...
> - Has read-write access to the entire GigaClaw project
>
> **Recommendation:** Use your personal "Message Yourself" chat or a solo Telegram group as your main channel. This ensures only you have admin control.
>
...
> You've chosen a group with other people. This means everyone in that group will have admin privileges over GigaClaw.
>
...
> Do you want to use your **personal chat** (message yourself) or a **Telegram group** as your main control channel?

For personal chat:

> Send any message to yourself in Telegram (the "Message Yourself" chat). Tell me when done.
...
> Do you want the agent to be able to access any directories **outside** the GigaClaw project?
>
...
```bash
mkdir -p ~/.config/gigaclaw
cat > ~/.config/gigaclaw/mount-allowlist.json << 'EOF'
{
  "allowedRoots": [],
  "blockedPatterns": [],
...
```bash
cat ~/.config/gigaclaw/mount-allowlist.json
```
...
> - Changes require restarting the GigaClaw service
>
...
cat > ~/Library/LaunchAgents/com.gigaclaw.plist << EOF
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key>
    <string>com.gigaclaw</string>
    ...
    <string>${PROJECT_PATH}/logs/gigaclaw.log</string>
    <key>StandardErrorPath</key>
    <string>${PROJECT_PATH}/logs/gigaclaw.error.log</string>
</dict>
</plist>
EOF
...
npm run build
mkdir -p logs
launchctl load ~/Library/LaunchAgents/com.gigaclaw.plist
```

Verify it's running:

```bash
launchctl list | grep gigaclaw
```
...
**Service not starting**: Check `logs/gigaclaw.error.log`
...
- Check `logs/gigaclaw.log` for errors

**Telegram disconnected**:

- The service will show a macOS notification
- Run `npm run auth` to re-authenticate
- Restart the service: `launchctl kickstart -k gui/$(id -u)/com.gigaclaw`

**Unload service**:

```bash
launchctl unload ~/Library/LaunchAgents/com.gigaclaw.plist
```
