...
Yes. GigaClaw uses Docker, which works on both macOS and Linux. Just install Docker and run `/setup`.
```
...
\`\`\`bash
docker images | grep gigaclaw-agent
...
```bash
echo '{}' | docker run -i --entrypoint /bin/echo gigaclaw-agent:latest "Container OK"
```
...
docker run --rm --entrypoint /bin/bash -v /tmp/test-ro:/test:ro gigaclaw-agent:latest \
...
```bash
mkdir -p /tmp/test-rw
docker run --rm --entrypoint /bin/bash -v /tmp/test-rw:/test gigaclaw-agent:latest \
