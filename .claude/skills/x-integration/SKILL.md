---
name: x-integration
description: X (Twitter) integration for GigaClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** GigaClaw v1.0.0. Directory structure may change in future versions.
...
1. **GigaClaw is installed and running** - Telegram connected, service active
...
# Verify: launchctl list | grep gigaclaw shows PID and exit code 0
```
...
| `GIGACLAW_ROOT` | `process.cwd()`                                                | Project root directory                   |
...
| `logs/gigaclaw.log`       | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into GigaClaw, make the following modifications:
...
All paths below are relative to project root (`GIGACLAW_ROOT`).
...
```bash
launchctl list | grep gigaclaw  # Should show PID and exit code 0 or -
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/gigaclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/gigaclaw.log | tail -20
```
...
container run gigaclaw-agent ls -la /app/src/skills/
```
