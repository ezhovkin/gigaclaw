...
| ---------------------- | -------------------------------------- | ----------------------------------------------- |
| **Main app logs**      | `logs/gigaclaw.log`                    | Host-side Telegram, routing, container spawning |
| **Main app errors**    | `logs/gigaclaw.error.log`              | Host-side errors                                |
...
echo '{}' | container run -i \
  ...
  --entrypoint /bin/bash gigaclaw-agent:latest \
...
```bash
container run --rm --entrypoint /bin/bash gigaclaw-agent:latest -c 'ls -la /workspace/'
```
...
```bash
container run --rm --entrypoint /bin/bash gigaclaw-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
container run --rm -it --entrypoint /bin/bash gigaclaw-agent:latest
```
...
# Also clear the session ID from GigaClaw's tracking
...
```bash
grep "Session initialized" logs/gigaclaw.log | tail -5
...
```bash
echo "=== Checking GigaClaw Container Setup ==="
...
container system status &>/dev/null && echo "OK" || echo "NOT RUNNING - GigaClaw should auto-start it; check logs"

echo -e "\n4. Container image exists?"
echo '{}' | container run -i --entrypoint /bin/echo gigaclaw-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
...
SESSIONS=$(grep "Session initialized" logs/gigaclaw.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
