...
description: Add voice message transcription to GigaClaw using OpenAI's Whisper API. Automatically transcribes voice notes so the agent can read and respond to them.
---

# Add Voice Message Transcription

This skill adds automatic voice message transcription using OpenAI's Whisper API. When users send voice notes, they'll be transcribed and the agent can read and respond to the content.
...
export async function transcribeAudioMessage(audioBuffer: Buffer): Promise<string | null> {
  ...
  try {
    ...
    console.log(`Audio message: ${audioBuffer.length} bytes`);
...
```bash
tail -f logs/gigaclaw.log | grep -i "voice\|transcri"
```
...
```bash
tail -100 logs/gigaclaw.log | grep -i transcription
```
