...
Adds Parallel AI MCP integration to GigaClaw for advanced web research capabilities.
...
- **Non-blocking Design** - Uses GigaClaw scheduler for result polling (no container blocking)
...
2. GigaClaw already set up and running
...
Add Parallel AI MCP servers after the gigaclaw server:
...
3. Create a polling scheduled task using `mcp__gigaclaw__schedule_task`:
```
...
3.  Send results to user with mcp**gigaclaw**send_message
4.  Use mcp**gigaclaw**complete_scheduled_task to mark this task as done
...
```bash
echo '{}' | container run -i --entrypoint /bin/echo gigaclaw-agent:latest "Container OK"
```
...
- Check task runs: `tail -f logs/gigaclaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.gigaclaw`
