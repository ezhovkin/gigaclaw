---
name: add-gmail
description: Add Gmail integration to GigaClaw. Can be configured as a tool (agent reads/sends emails when triggered from Telegram) or as a full channel (emails can trigger the agent, schedule tasks, and receive replies). Guides through GCP OAuth setup and implements the integration.
---

# Add Gmail Integration

This skill adds Gmail capabilities to GigaClaw. It can be configured in two modes:
...
> How do you want to use Gmail with GigaClaw?
>
> **Option 1: Tool Mode**
>
> - Agent can read and send emails when you ask it to
> - Triggered only from Telegram (e.g., "@Neo check my email" or "@Neo send an email to...")
...
>    - If prompted for consent screen, choose "External", fill in app name (e.g., "GigaClaw"), your email, and save
>    - For Application type, select **Desktop app**
>    - Name it anything (e.g., "GigaClaw Gmail")
>    - Click **Create**
...
```bash
launchctl kickstart -k gui/$(id -u)/com.gigaclaw
```

Check that it started:

```bash
sleep 2 && launchctl list | grep gigaclaw
```
...
>
> `@Neo check my recent emails`
>
> Or:
>
> `@Neo list my Gmail labels`
...
>
> - Create a Gmail label (e.g., "GigaClaw")
...
>
> - Emails with a subject starting with a keyword (e.g., "[Neo]")
...
Complete all Tool Mode steps above before continuing. Verify Gmail tools work by having the user test `@Neo check my recent emails`.
...
export const EMAIL_CHANNEL: EmailChannelConfig = {
  ...
  triggerValue: 'GigaClaw', // the label name, address pattern, or prefix
  contextMode: 'thread',
...
export async function sendEmailReply(
  ...
  const prefixedBody = EMAIL_CHANNEL.replyPrefix ? `${EMAIL_CHANNEL.replyPrefix}${body}` : body;
...
```bash
sleep 3 && tail -20 logs/gigaclaw.log | grep -i email
```
...
```bash
tail -f logs/gigaclaw.log | grep -E "(email|Email)"
```
