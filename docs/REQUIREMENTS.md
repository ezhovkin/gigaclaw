...
GigaClaw gives you the core functionality without that mess.
...
- Use existing tools (Telegram bot library, Claude Agent SDK, MCP servers)
- Minimal glue code
...
- Trigger: `@Neo` prefix (case insensitive), configurable via `ASSISTANT_NAME` env var
...
- Messages stored in SQLite, processed by router
- Bot token authentication during setup

### Scheduler

- Built-in scheduler runs on the host, spawns containers for task execution
- Custom `gigaclaw` MCP server (inside container) provides scheduling tools
...
**GigaClaw** - A reference to Clawdbot (now OpenClaw).
