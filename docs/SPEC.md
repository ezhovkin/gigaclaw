...
│  │    • mcp__gigaclaw__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── gigaclaw.log               # Host stdout
│   └── gigaclaw.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.gigaclaw.plist         # macOS service configuration
```
...
- `{{PROJECT_ROOT}}` - Absolute path to your gigaclaw installation
...
GigaClaw uses a hierarchical memory system based on CLAUDE.md files.
...
4. Router checks:
   ...
   │
   ▼
5. Router catches up conversation:
   ├── Fetch all messages since last agent interaction
   ├── Format with timestamp and sender name
   └── Build prompt with full conversation context
   │
   ▼
6. Router invokes Claude Agent SDK:
   ├── cwd: groups/{group-name}/
   ├── prompt: conversation history + current message
   ├── resume: session_id (for continuity)
   └── mcpServers: gigaclaw (scheduler)
   │
   ▼
7. Claude processes message:
...
Messages must start with the trigger pattern (default: `@Neo`):

- `@Neo what's the weather?` → ✅ Triggers Claude
- `@andy help me` → ✅ Triggers (case insensitive)
- `Hey @Neo` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @Neo what toppings do you recommend?
```
...
| ---------------------- | -------------------------- | -------------- |
| `@Assistant [message]` | `@Neo what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command                          | Example                            | Effect                 |
| -------------------------------- | ---------------------------------- | ---------------------- |
| `@Assistant add group "Name"`    | `@Neo add group "Family Chat"`     | Register a new group   |
| `@Assistant remove group "Name"` | `@Neo remove group "Work Team"`    | Unregister a group     |
| `@Assistant list groups`         | `@Neo list groups`                 | Show registered groups |
| `@Assistant remember [fact]`     | `@Neo remember I prefer dark mode` | Add to global memory   |

---

## Scheduled Tasks

GigaClaw has a built-in scheduler that runs tasks as full agents in their group's context.
...
```
User: @Neo remind me every Monday at 9am to review the weekly metrics

Claude: [calls mcp__gigaclaw__schedule_task]
        {
...
```
User: @Neo at 5pm today, send me a summary of today's emails
...
- `@Neo list my scheduled tasks` - View tasks for this group
- `@Neo pause task [id]` - Pause a task
- `@Neo resume task [id]` - Resume a paused task
- `@Neo cancel task [id]` - Delete a task

From main channel:

- `@Neo list all tasks` - View tasks from all groups
- `@Neo schedule task for "Family Chat": [prompt]` - Schedule for another group

---

## MCP Servers

### GigaClaw MCP (built-in)

The `gigaclaw` MCP server is created dynamically per agent call with the current group's context.
...
GigaClaw runs as a single macOS launchd service.

### Startup Sequence

When GigaClaw starts, it:
...
### Service: com.gigaclaw

**launchd/com.gigaclaw.plist:**
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/gigaclaw.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/gigaclaw.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.gigaclaw.plist ~/Library/LaunchAgents/
...
| ---------------------------------------- | ------------------------------- | ---------------------------------------------------------------------------------------- | -------------- |
| No response to messages                  | Service not running             | Check `launchctl list                                                                    | grep gigaclaw` |
| "Claude Code process exited with code 1" | Apple Container failed to start | Check logs; GigaClaw auto-starts container system but may fail                           |
| "Claude Code process exited with code 1" | Session mount path wrong        | Ensure mount is to `/home/node/.claude/` not `/root/.claude/`                            |
| Session not continuing                   | Session ID not saved            | Check `data/sessions.json`                                                               |
| Session not continuing                   | Mount path mismatch             | Container user is `node` with HOME=/home/node; sessions must be at `/home/node/.claude/` |
| "No groups registered"                   | Haven't added groups            | Use `@Neo add group "Name"` in main                                                      |

### Log Location

- `logs/gigaclaw.log` - stdout
- `logs/gigaclaw.error.log` - stderr
