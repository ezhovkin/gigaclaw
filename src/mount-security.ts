/**
 * Mount Security Module for GigaClaw
 *
 * Validates additional mounts against an allowlist stored OUTSIDE the project root.
 * This prevents container agents from modifying security configuration.
 *
 * Allowlist location: ~/.config/gigaclaw/mount-allowlist.json
 */
import fs from 'fs';
  ...
  for (const mount of mounts) {
    ...
    if (result.allowed) {
      validatedMounts.push({
        hostPath: result.realHostPath ?? mount.hostPath,
        containerPath: `/workspace/extra/${mount.containerPath}`,
        readonly: result.effectiveReadonly ?? true,
      });
