...
import { registeredGroups, registerGroup } from './state.js';
...
interface IpcTaskControlData {
  type: 'pause_task' | 'resume_task' | 'cancel_task';
  taskId: string;
}

interface IpcRegisterGroupData {
  type: 'register_group';
  jid: string;
  name: string;
  folder: string;
...
type IpcData = IpcMessageData | IpcScheduleTaskData | IpcTaskControlData | IpcRegisterGroupData;
            ...
            try {
              ...
              if (data.type === 'message' && data.chatJid && data.text) {
                ...
                if (isMain || (targetGroup && targetGroup.folder === sourceGroup)) {
                  await sendMessage(
                    data.chatJid,
                    `${MESSAGE_PREFIX}${ASSISTANT_NAME}: ${data.text}`
                  );
        ...
        if (fs.existsSync(tasksDir)) {
          ...
          for (const file of taskFiles) {
            ...
            try {
              ...
              // Strip invalid JSON escape sequences (e.g. \! from bash)
...
async function processTaskIpc(data: IpcData, sourceGroup: string, isMain: boolean): Promise<void> {
  ...
  switch (data.type) {
    case 'schedule_task': {
      ...
      if (data.prompt && data.prompt.length > MAX_PROMPT_SIZE) {
        logger.warn(
          { size: data.prompt.length, max: MAX_PROMPT_SIZE },
          'Task prompt too large, rejecting'
        );
        break;
      }
      // Accept target_group as alias for groupFolder (agent may use either)
      const groupFolder = data.groupFolder || data.target_group;
      if (data.prompt && data.schedule_type && data.schedule_value && groupFolder) {
        // Authorization: non-main groups can only schedule for themselves
        const targetGroup = groupFolder;
