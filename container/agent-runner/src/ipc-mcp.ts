/**
 * IPC-based MCP Server for GigaClaw
...
export function createIpcMcp(ctx: IpcMcpContext) {
  ...
  return createSdkMcpServer({
    ...
    tools: [
      tool(
        'send_message',
        'Send a message to the current group. Use this to proactively share information or updates.',
        {
...
• once: Local time WITHOUT "Z" suffix (e.g., "2026-02-01T15:30:00"). Do NOT use UTC/Z suffix.`,
        ...
        async (args) => {
          ...
          if (args.schedule_type === 'cron') {
            ...
            let date = new Date(args.schedule_value);
            // Accept Unix timestamps (seconds or milliseconds)
            if (isNaN(date.getTime())) {
              const num = Number(args.schedule_value);
              if (!isNaN(num)) {
            ...
            if (isNaN(date.getTime())) {
              return {
                content: [
                  {
                    type: 'text',
                    text: `Invalid timestamp: "${args.schedule_value}". Use ISO 8601 format like "2026-02-01T15:30:00" or Unix timestamp.`,
                  },
                ],
                isError: true,
              };
            }
            // Normalize to ISO string for downstream processing
...
Use available_groups.json to find the JID for a group. The folder name should be lowercase with hyphens (e.g., "family-chat").`,
        {
          jid: z.string().describe('The group JID (e.g., "telegram@chat:123456789")'),
          name: z.string().describe('Display name for the group'),
          folder: z
            .string()
            .describe('Folder name for group files (lowercase, hyphens, e.g., "family-chat")'),
          trigger: z.string().describe('Trigger word (e.g., "@Neo")'),
        },
        async (args) => {
          if (!isMain) {
            return {
